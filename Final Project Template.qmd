...
In this experiment, our group asks the question of whether or not an input device has a significant effect on the reaction time of users. Our null hypothesis, or the baseline assumption we make, is that the input device one uses has no effect on their mean reaction time. Our alternative hypothesis, what we are testing, is that there is some significant difference in mean reaction times between input devices. The intent of our experiment aims at identifying if, on average, the population of people on devices can react quicker on their phone, their laptop, or their laptop. 

# Experimental Design

The design we will use to test this hypothesis is a Random Component Block Design (RCBD). 

Firstly, we identify that we will be blocking by input device. Block 1: Phone, Block 2: Tablet, Block 3: Laptop. We choose 11 participants as our experimental units. We implement the repetition of each trial 10 times within each block before moving on to the next block. 

To achieve homogenous treatments, all 11 participants will do their trials on each of the 3 devices. The order of devices used for each participant, the treatment, is randomized. The order in which treatments are assigned to participants is random. Each participant performing a trial will have their reaction time recorded in milliseconds in an excel spreadsheet. 

We identify that in this experiment, we have two fixed factor variables: input devices (which has 3 levels: phone, laptop, tablet) and 10 fixed trials per device per participant, and a random factor variable: participants. The total number of trials is given by blocks x experimental units x repetitions; in this case we have 3 blocks, 11 experimental units and 10 repetitions.
...
basic_stats %>% kable(caption = "Basic Statistics by Treatment Group")
```
...
```{r anova-output}
as.data.frame(anova_table[[1]]) %>% kable(caption = "ANOVA Table for RCBD Model")

data.frame(
  F_statistic_Treatment = round(f_stat, 4),
  P_value = format.pval(p_value, digits = 4),
  Treatment_df = anova_table[[1]]["Treatment", "Df"],
  Error_df = anova_table[[1]]["Residuals", "Df"]
) %>% kable(caption = "Critical Test Statistics")
```
...
data.frame(
  Assumption = c(paste0("Normality (Shapiro-Wilk): W = ", round(shapiro_test$statistic, 4)), paste0("Equal Variances (Bartlett): K² = ", round(bartlett_test$statistic, 4)), "Independence"),
  Statistic  = c(round(shapiro_test$statistic,4),
                 round(bartlett_test$statistic,4),
                 ""),
  P_value    = c(format.pval(shapiro_test$p.value, 3),
                 format.pval(bartlett_test$p.value, 3),
                 ""),
  Decision   = c(ifelse(shapiro_test$p.value>0.05,"Pass","Check"),
                 ifelse(bartlett_test$p.value>0.05,"Pass","Fail"),
                 "Ensured by randomized treatment order and blocking structure.")
) %>% kable(caption = "Formal Test of Assumptions")
```
...
Human Benchmark. (n.d.). Reaction Time Test. https://humanbenchmark.com/tests/reactiontime
...
### Code Snippet: Intiail Statistics
